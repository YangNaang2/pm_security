...
본 프로젝트는 보수적인 안전 수칙 판별 기준을 적용하여, **정상 이용자를 위반자로 오인하는 치명적인 오류(False Positive)를 원천 차단**하는 데 성공했습니다. 
...
* **Layer 2 (공간 필터링):** 2D 이미지 기반 기하학적 연산의 태생적 한계로 인해, 다인승 위반자들의 발끝 좌표가 카메라 앵글 상 킥보드 하단 영역 내에서 하나로 겹쳐 보일 경우 알고리즘이 이를 1인 탑승으로 오인(False Negative)할 수 있습니다.
...
## 3️⃣ Layer 2 최종 혼동 행렬 (Confusion Matrix)
