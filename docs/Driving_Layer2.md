...
| :--- | :---: | :---: | :---: |
| **정상 주행 (NORMAL / 36장)** | **35 장** <br>*(True Normal)* | **1 장** <br>*(False Positive)* | **97.2%** |
| **위반 주행 (ABNORMAL / 29장)** | **25 장** <br>*(False Negative)* | **4 장** <br>*(True Abnormal)* | **13.8 %** |

### 💡 한계점 및 로직 타당성 고찰
본 프로젝트는 보수적인 기준을 적용하여, **정상 이용자를 위반자로 오인하는 치명적인 오류(False Positive)를** 최대한 차단하는 데 성공했습니다. 

1. **미탐지 원인 (False Negative):** 두 탑승자가 완벽히 밀착되거나 다리가 벌어진 기형적 자세 등, 절대적인 위반 이미지 학습량 부족 및 2D 원근법의 태생적 한계로 인해 일부 위반 차량을 정상으로 오인하는 경우가 존재합니다. 또한 헬멧 이미지 학습시 대부분의 위반 주행 이미지는 모자이크 처리가 되어있기에 클린한 이미지를 구하는데 어려움이 있어 학습에 제한이 있었습니다.
2. **실서비스 타당성:** 공유 모빌리티 자동 단속 시스템에서 선량한 이용자에게 패널티가 부과되는 시스템 오류(`False Positive`)는 서비스 신뢰도에 치명적입니다. 따라서 특이 케이스 위반자를 일부 놓치더라도(`False Negative`), **정상 사용자를 철저하게 보호하고자 하는 기존의 목적은 성공**했습니다.
